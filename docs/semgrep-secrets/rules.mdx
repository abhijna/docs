...
## Write a rule[​](#write-a-rule "Direct link to Write a rule")
...
### Create a YAML file[​](#create-a-yaml-file "Direct link to Create a YAML file")

If you're familiar with Semgrep's rules syntax, including the [validator syntax](/docs/semgrep-secrets/validators), you can create a YAML file containing your rules. When you're done, [publish your rules for use with your organization](/docs/writing-rules/private-rules).
...
### Use Semgrep Editor[​](#use-semgrep-editor "Direct link to Use Semgrep Editor")
...
## Sample rule[​](#sample-rule "Direct link to Sample rule")

The following sample rule detects a leaked GitHub personal access token (PAT):

```
rules:- id: github_example  message: >-    This is an example rule, that performs validation against github.com  severity: MEDIUM  languages:  - regex  validators:  - http:      request:        headers:          Authorization: Bearer $REGEX          Host: api.github.com          User-Agent: Semgrep        method: GET        url: https://api.github.com/user      response:      - match:        - status-code: 200        result:          validity: valid      - match:        - status-code: 401        result:          validity: invalid  patterns:  - patterns:    - pattern-regex: (?<REGEX>\b((ghp|gho|ghu|ghs|ghr|github_pat)_[a-zA-Z0-9_]{36,255})\b)    - focus-metavariable: $REGEX    - metavariable-analysis:        analyzer: entropy        metavariable: $REGEX
```

This can also be done in any Semgrep-supported language:

```
rules:- id: github_example  message: >-    This is an example rule that performs validation against github.com  severity: MEDIUM  languages:  - javascript  - typescript  validators:  - http:      request:        headers:          Authorization: Bearer $REGEX          Host: api.github.com          User-Agent: Semgrep        method: GET        url: https://api.github.com/user      response:      - match:        - status-code: 200        result:          validity: valid      - match:        - status-code: 401        result:          validity: invalid  patterns:  - patterns:    - pattern: |        "$R"    - metavariable-pattern:        metavariable: $R        patterns:          - pattern-regex: (?<REGEX>\b((ghp|gho|ghu|ghs|ghr|github_pat)_[a-zA-Z0-9_]{36,255})\b)    - focus-metavariable: $REGEX    - metavariable-analysis:        analyzer: entropy        metavariable: $REGEX
```

### Subkeys under the `metadata` key[​](#subkeys-under-the-metadata-key "Direct link to subkeys-under-the-metadata-key")

These subkeys provide context to both you and other end-users, as well as to Semgrep.

```
  ...  metadata:    ...    secret_type: GitHub    technology:    - secrets  ...
```
...
### Subkeys under the `patterns` key[​](#subkeys-under-the-patterns-key "Direct link to subkeys-under-the-patterns-key")

These subkeys identify the token to analyze in a given match.

```
  ...  patterns:  ...  - pattern-regex: (?<REGEX>\b((ghp|gho|ghu|ghs|ghr|github_pat)_[a-zA-Z0-9_]{36,255})\b)  - focus-metavariable: $REGEX  - metavariable-analysis:      analyzer: entropy      metavariable: $REGEX  ..
```
...
<Check>
  For more information, see the rule syntax for [Focus metavariable](/docs/writing-rules/rule-syntax#focus-metavariable).
</Check>

### Subkeys under the `validators` and `http` keys[​](#subkeys-under-the-validators-and-http-keys "Direct link to subkeys-under-the-validators-and-http-keys")

The `validators` key uses a list of keys to define the validator function. In particular, the `http` key defines how the rule forms a request object and what response is expected for valid and invalid states. Although some rules do not use a `validators` key, most Secrets rules use it.

```
  ...  validators:  - http:      request:        headers:          Authorization: Bearer $REGEX          Host: api.github.com          User-Agent: Semgrep        method: GET        url: https://api.github.com/user      response:        - match:          - status-code: '200'          result:            validity: valid        - match:          - status-code: '404'          result:            validity: invalid
```
...
<Check>
  See [Validators](/docs/semgrep-secrets/validators) for more information.
</Check>

## Metavariable binding[​](#metavariable-binding "Direct link to Metavariable binding")
...
[Message displays metavariable content](https://semgrep.dev/embed/editor?snippet=JDzRR)
...
## Differences between Semgrep Secrets rules and Semgrep Registry rules[​](#differences-between-semgrep-secrets-rules-and-semgrep-registry-rules "Direct link to Differences between Semgrep Secrets rules and Semgrep Registry rules")
...
[Edit this page](https://github.com/semgrep/semgrep-docs/edit/main/docs/semgrep-secrets/rules.md)
