...
## Triage findings[​](#triage-findings "Direct link to Triage findings")
...
3. Applying a **triage state** to the filtered findings based on the analysis in step 2.
   ...
   2. Setting or retaining a finding as **Open**, **Reviewing**, or **Fixing** means that the finding is a true positive and needs to be fixed or resolved.
      1. Optional: You can [create a ticket in Jira](/docs/semgrep-appsec-platform/jira) to assign a developer to fix findings.
...
## Common filtering use cases[​](#common-filtering-use-cases "Direct link to Common filtering use cases")

You can find and perform bulk operations through filtering; [all filter operations](/docs/semgrep-secrets/findings#filter-findings) are available to you on the **Secrets** page.
...
* You have set up [comments](/docs/category/pr-or-mr-comments) as part of your core deployment.
* You have defined which rules and validation states should be in Allow, Comment, or Block mode in the [Policies](/docs/semgrep-secrets/policies) page.

<Info>
  Define which rules and validation states should be in Allow, Comment, or Block mode in the [Policies](/docs/semgrep-secrets/policies) page.
</Info>

## Default Secrets page view and branch logic[​](#default-secrets-page-view-and-branch-logic "Direct link to Default Secrets page view and branch logic")

In Semgrep, a **single** finding may appear in several branches. These appearances are called **instances** of a finding. In Semgrep Secrets, the **latest instance**, or the finding from the most recent branch scanned, is displayed by default. This is because, if a Secrets finding is present in **any branch**, even a non-primary (default) branch, it is considered [valid](/docs/semgrep-secrets/conceptual-overview#validate-secrets).
...
[Edit this page](https://github.com/semgrep/semgrep-docs/edit/main/docs/semgrep-secrets/triage-remediation.md)
