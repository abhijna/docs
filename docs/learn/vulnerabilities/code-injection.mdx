...
## What Is Code Injection?[​](#what-is-code-injection "Direct link to What Is Code Injection?")
...
## Common Code Injection Attacks[​](#common-code-injection-attacks "Direct link to Common Code Injection Attacks")
...
```
from flask import Flask, requestapp = Flask(__name__)@app.route("/sum")def sum_numbers():    numbers = request.args.get("numbers")    numbers_list = eval(numbers)    total = 0    for num in numbers_list:        total += num    return f"Sum is {total}"
```
...
The `eval` function quickly turns this string into a Python list-object, and the resulting list is easy to work with for the developer. But since the application uses `eval`, an attacker could instead send a malicious string instead:
...
## Detecting Code Injection in Your Code[​](#detecting-code-injection-in-your-code "Direct link to Detecting Code Injection in Your Code")

Here is what that vulnerable web application might look like in simplified form:

```
from flask import Flask, requestapp = Flask(__name__)@app.route("/route")def my_route():    input_string = request.args.get("string")    eval(input_string)
```
...
[Edit this page](https://github.com/semgrep/semgrep-docs/edit/main/docs/learn/vulnerabilities/code-injection/overview.md)
