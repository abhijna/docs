...
## What is XML Security?[​](#what-is-xml-security "Direct link to What is XML Security?")
...
### External Document Type Definition[​](#external-document-type-definition "Direct link to External Document Type Definition")
...
```
<!DOCTYPE sast_tool [ <!ELEMENT sast_tool (name, fp)> <!ELEMENT name (#PCDATA)> <!ELEMENT fp (#PCDATA)>]>
```
...
### External XML Entities[​](#external-xml-entities "Direct link to External XML Entities")
...
```
<!ENTITY semgrep "Semgrep" ><!ENTITY best_sast_tool "&semgrep;" >
```
...
The XML specification includes more than just DTDs and external entities for including external data. Other features like XInclude, the `schemaLocation` attribute, the `xsl:include` element, the `document()` function, and `import` or `include` statements can all be used to reference external resources. You can find an overview of these and more in our [XML Cheat Sheet](/docs/cheat-sheets/java-xxe).

## Common XML Attacks[​](#common-xml-attacks "Direct link to Common XML Attacks")
...
### XML Injection[​](#xml-injection "Direct link to XML Injection")
...
```
<user>  <name>pieter</name>  <admin>false</admin></user>
```

If the application does not validate input properly, an attacker could send:

```
<user>  <name>pieter</name>  <admin>true</admin></user>
```
...
### Exponential Entity Expansion (XEE)[​](#exponential-entity-expansion-xee "Direct link to Exponential Entity Expansion (XEE)")
...
### XML External Entity (XXE) Injection[​](#xml-external-entity-xxe-injection "Direct link to XML External Entity (XXE) Injection")
...
## Detecting XML Security Vulnerabilities in Your Code[​](#detecting-xml-security-vulnerabilities-in-your-code "Direct link to Detecting XML Security Vulnerabilities in Your Code")
...
```
DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();DocumentBuilder builder = factory.newDocumentBuilder();Document doc = builder.parse("input.xml");
```
...
### Consider using alternative formats[​](#consider-using-alternative-formats "Direct link to Consider using alternative formats")
...
### Use up-to-date language and libraries[​](#use-up-to-date-language-and-libraries "Direct link to Use up-to-date language and libraries")
...
### Disable external entities[​](#disable-external-entities "Direct link to Disable external entities")
...
### Disabling external schema/stylesheet processing[​](#disabling-external-schemastylesheet-processing "Direct link to Disabling external schema/stylesheet processing")
...
[Edit this page](https://github.com/semgrep/semgrep-docs/edit/main/docs/learn/vulnerabilities/xml-security/overview.md)
