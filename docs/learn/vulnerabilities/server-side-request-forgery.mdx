...
## What is SSRF?[​](#what-is-ssrf "Direct link to What is SSRF?")
...
## Common SSRF Attacks[​](#common-ssrf-attacks "Direct link to Common SSRF Attacks")
...
## Detecting SSRF Vulnerabilities in Your Code[​](#detecting-ssrf-vulnerabilities-in-your-code "Direct link to Detecting SSRF Vulnerabilities in Your Code")

Let’s look at a short Python example:

```
import requestsfrom flask import requestdef fetch_data_vulnerable():    user_supplied_url = request.args.get("url")    if user_supplied_url.startswith("https://semgrep.dev")	    response = requests.get(user_supplied_url)	  else		  response = None    return response.text    def fetch_data_securely():    user_supplied_request_params = request.args.get("params")    response = requests.get("https://semgrep.dev/" + user_supplied_request_params)    return response.text
```
...
[Edit this page](https://github.com/semgrep/semgrep-docs/edit/main/docs/learn/vulnerabilities/server-side-request-forgery/overview.md)
