...
* [sql-injection](/docs/tags/sql-injection)
...
## What is SQL Injection (SQLi)[​](#what-is-sql-injection-sqli "Direct link to What is SQL Injection (SQLi)")
...
## Common SQL Injection Attacks[​](#common-sql-injection-attacks "Direct link to Common SQL Injection Attacks")
...
```
def search(request):    query = request.GET['q']    sql = f"SELECT * FROM semgrep_customers WHERE company = '{query}';"    cursor = db.cursor()    cursor.execute(sql)
```
...
### Classic Injection via string concatenation[​](#classic-injection-via-string-concatenation "Direct link to Classic Injection via string concatenation")
...
### Blind SQL Injection[​](#blind-sql-injection "Direct link to Blind SQL Injection")
...
### Second‑order SQL Injection[​](#secondorder-sql-injection "Direct link to Second‑order SQL Injection")
...
## Detecting SQL Injection Vulnerabilities in Your Code[​](#detecting-sql-injection-vulnerabilities-in-your-code "Direct link to Detecting SQL Injection Vulnerabilities in Your Code")
...
### Example Semgrep Rule: express-sequelize-injection[​](#example-semgrep-rule-express-sequelize-injection "Direct link to Example Semgrep Rule: express-sequelize-injection")

Consider this JavaScript / TypeScript / Express + Sequelize code:

```
let criteria = req.query.foo// ok: this is proper handling that should not matchsequelize.query(  'SELECT * FROM projects WHERE status = ?',  {    replacements: [req.body.foo],    type: QueryTypes.SELECT  })// Unsafe: direct interpolation / template literalsequelize.query(`SELECT * FROM Foo WHERE criteria LIKE '%${criteria}%'`);// Unsafe: using replacement in object but still embedding unsanitized user datasequelize.query(`SELECT * FROM Foo WHERE criteria LIKE '%${obj.replacements[0]}%'`);
```
...
### Always use parameterized queries[​](#always-use-parameterized-queries "Direct link to Always use parameterized queries")
...
```
cursor.execute(    "SELECT * FROM some_table WHERE title LIKE %s",    [request.GET['q']])
```
...
### Avoid raw string construction in queries[​](#avoid-raw-string-construction-in-queries "Direct link to Avoid raw string construction in queries")
...
### Make the safe path the default[​](#make-the-safe-path-the-default "Direct link to Make the safe path the default")
...
### Audit third-party query extensions[​](#audit-third-party-query-extensions "Direct link to Audit third-party query extensions")
...
### Check stored procedures too[​](#check-stored-procedures-too "Direct link to Check stored procedures too")
...
### Limit database privileges[​](#limit-database-privileges "Direct link to Limit database privileges")
...
### Catch Database Errors[​](#catch-database-errors "Direct link to Catch Database Errors")
...
[Edit this page](https://github.com/semgrep/semgrep-docs/edit/main/docs/learn/vulnerabilities/sql-injection/overview.md)
