...
This document describes Semgrep’s pattern syntax. You can also see pattern [examples by language](/docs/writing-rules/pattern-examples). In the command line, patterns are specified with the flag `--pattern` (or `-e`). Multiple coordinating patterns may be specified in a configuration file. See [rule syntax](/docs/writing-rules/rule-syntax) for more information.
